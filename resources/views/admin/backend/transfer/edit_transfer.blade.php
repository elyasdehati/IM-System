...
<div class="content d-flex flex-column flex-column-fluid">
 ...
 <div class="card">
    <div class="card-body">
    <form action="{{ route('update.transfer',$editData->id)}}" method="post" enctype="multipart/form-data">
       @csrf
 ...
 <div class="col-xl-12">
    <div class="card">
       <div class="row">
        ...
        <input type="hidden" name="from_warehouse_id" value="{{ $editData->from_warehouse_id }}">
        <input type="hidden" name="to_warehouse_id" value="{{ $editData->to_warehouse_id }}">

          <div class="col-md-4 mb-3">
                <div class="form-group w-100">
                <label class="form-label" for="formBasic">From Warehouse : <span class="text-danger">*</span></label>
                <select name="from_warehouse_id" id="warehouse_id" class="form-control form-select" disabled>
    <option value="">Select Warehouse</option>
    @foreach ($warehouses as $item)
    <option value="{{ $item->id }}" {{ $editData->from_warehouse_id == $item->id ? 'selected' : '' }} >{{ $item->name }}</option>
    @endforeach
                </select>
          ...
          <div class="col-md-4 mb-3">
                ...
                <select name="to_warehouse_id" id="warehouse_id" class="form-control form-select" disabled>
    <option value="">Select Warehouse</option>
    @foreach ($warehouses as $item)
    <option value="{{ $item->id }}" {{ $editData->to_warehouse_id == $item->id ? 'selected' : '' }} >{{ $item->name }}</option>
    @endforeach
                </select>
                <small id="warehouse_error" class="text-danger d-none">Please select the first warehouse.</small>
                </div>
  ...
  <div class="row">
    ...
    @foreach ($editData->transferItems as $item)
    <tr data-id={{ $item->id  }}>
         ...
         <div class="col-md-4">
            <div class="form-group w-100">
               ...
               <select name="status" id="status" class="form-control form-select">
                  ...
                  <option value="Transfer" {{ $editData->status == 'Transfer' ? 'selected' : '' }} >Transfer</option>
...
{{-- <script>
    ...
    // Paste this once (e.g., at the end of your current <script>) to auto-update Due when Paid changes
    (function () {
    ...
    const grandTd = document.getElementById('grandTotal');
    if (!paidInput || !dueTd || !dueHidden || !grandTd) return;
    ...
    const currencyPrefix = (() => {
        const t = grandTd.textContent || '';
        const m = t.match(/^[^\d\-]*/);
        return (m && m[0]) ? m[0].trim() + (m[0].trim() ? ' ' : '') : '';
    })();

    function updateDue() {
        const grandTotal = parseMoney(grandTd.textContent);
        const paid = parseMoney(paidInput.value);
        let due = grandTotal - paid;
        if (due < 0) due = 0;
        const formatted = due.toFixed(2);
        dueTd.textContent = `${currencyPrefix}${formatted}`;
        dueHidden.value = formatted;
    }

    ['input', 'change', 'keyup'].forEach(evt =>
        paidInput.addEventListener(evt, updateDue)
    );

    // also recalc if Grand Total text changes elsewhere in the page
    new MutationObserver(updateDue).observe(grandTd, { childList: true, subtree: true, characterData: true });
